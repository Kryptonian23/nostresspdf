...
let
  ...
  hushpdf-static = stdenv.mkDerivation {
    pname = "hushpdf-static";
    version = "0.1.0";
...
stdenv.mkDerivation {
  ...
  installPhase = let
    nginxConf = builtins.toFile "hushpdf-nginx.conf" ''
      daemon off;
      worker_processes 1;
  ...
  in ''
    ...
    mkdir -p $out/share/hushpdf
    cp -r ${hushpdf-static}/* $out/share/hushpdf/

    # Decompress LibreOffice WASM .gz files
    if [ -d $out/share/hushpdf/libreoffice-wasm ]; then
      cd $out/share/hushpdf/libreoffice-wasm
      for f in *.gz; do
        if [ -f "$f" ]; then
    ...
    mkdir -p $out/etc/hushpdf
    sed -e "s|HUSHPDF_ROOT|$out/share/hushpdf|g" \
        -e "s|NGINX_MIME_TYPES|${nginx}/conf/mime.types|g" \
        ${nginxConf} > $out/etc/hushpdf/nginx.conf

    # Install run script
    mkdir -p $out/bin
    cat > $out/bin/hushpdf <<'WRAPPER'
#!/bin/sh
HUSHPDF_PORT=''${HUSHPDF_PORT:-3000}
HUSHPDF_CONF="@out@/etc/hushpdf/nginx.conf"
RUNTIME_CONF=$(mktemp /tmp/hushpdf-nginx.XXXXXX.conf)

sed "s|listen 3000|listen $HUSHPDF_PORT|g" "$HUSHPDF_CONF" > "$RUNTIME_CONF"
...
WRAPPER
    ...
    substituteInPlace $out/bin/hushpdf \
      --replace-fail "@out@" "$out" \
      --replace-fail "@nginx@" "${nginx}"
    chmod +x $out/bin/hushpdf
